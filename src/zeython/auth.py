...
keyed off ``APP_SECRET_KEY``) holding the authenticated user's ID, an
:class:`AuthManager` that knows how to look up and verify credentials against
whichever model you designate as your user model, and a handful of functions
(`login`, `logout`, `current_user`, `require_auth`) that operate on a request.
...
class AuthServiceProvider(ServiceProvider):
    ...
    ``APP_SECRET_KEY``, so that must be set) and binds an :class:`AuthManager`
    into the container::
    ...
    def boot(self) -> None:
        ...
        self.app.add_middleware(
            ...
            session_cookie=self.config.get("session.cookie_name", "zeython_session"),

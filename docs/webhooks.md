...
[Notifications](notifications.md), which notify a recipient *inside* your
own app instead.
...
[the background job queue](queues.md) for delivery/retries, HMAC-SHA256
...
goes through whichever `Queue` it binds (see [Background Jobs](queues.md)).

## Your own models

Neither model is mandated by the framework -- the same `record_model`
pattern [Audit Logging](audit-log.md) and [Notifications](notifications.md)
...
they don't control -- `fire_webhook()` will happily deliver a
server-signed POST wherever that row says to, including a private/internal
address (`http://169.254.169.254/...`, `http://localhost:6379`) if nothing
stops it: a classic SSRF vector. Validate the URL yourself before saving
it (a public scheme/host allow-list, resolving and rejecting private IP
ranges) the same way you'd validate any other user-submitted URL your
server will later fetch -- `zeython.webhooks` sends what a row's `url`
says, it doesn't audit where that points.
...
`UserRegistered` event [Events](events.md) already dispatches -- add your
...
If your own app is on the *receiving* end of someone else's webhook (or
another Zeython app's), verify it the same way before trusting the body:
...
job gets (see [Background Jobs](queues.md#retries-and-backoff)). If `delivery_model`
...
[Notifications](notifications.md) applies per channel.

...
Generating the key is the client's job, not the server's — usually a
UUID minted once per logical operation (e.g. once when a "place order"
button is clicked, reused across every retry of that same click, but
never reused for the *next* order).
...
Records are scoped per method *and* path, so the same key value used
...
[`RateLimiter`](rate-limiting.md#the-default-limiter-is-process-local) and
[`Cache`](caching.md) already document for their default backends, not
something new here.

## The default store is process-local

`IdempotencyServiceProvider` uses its own
[`InMemoryCache`](caching.md) by default — correct for a single process,
...
request again. Share a [`RedisCache`](redis.md) instead for a store every
...
- The request body is buffered in memory to compute its hash (for the
  ...
  trade-off [`ETagServiceProvider`](api-standards.md#conditional-requests-etags)
  makes on the response side. Fine for a typical JSON request body; not a
  fit in front of a large streamed upload.
- There's no automatic cleanup beyond `ttl` — a very high-cardinality key
  space (a fresh UUID per request, correctly used) naturally self-limits
  since old entries simply expire; this isn't a request-deduplication
  cache meant to hold every request ever made indefinitely.

## API reference

See [`zeython.idempotency`](reference/http-api.md) for the full API.

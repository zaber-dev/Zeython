...
Subclass `Storage` and implement `put`/`get`/`delete`/`exists`/`url` — five
async methods (`url` is sync, since it's typically pure string formatting).
Bind an instance the same way as `S3Storage` above.

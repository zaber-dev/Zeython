...
Enrollment is two steps, both requiring the user to already be logged in:
generate a secret, then confirm it with a live code from the app that just
scanned it — proving the user actually finished setting it up before MFA
starts gating their login.
...
[`current_user()`](authentication.md)/`require_auth()` keep returning
...
challenge in place, so the client can retry (throttle the endpoint the
same way you'd throttle `/login` — see [Rate Limiting](rate-limiting.md)).
...
Clears the secret, the enabled flag, and every recovery code. A real app
should require re-entering the password (or a fresh TOTP code) before
calling this — an attacker who's already hijacked a session shouldn't be
able to turn off the account's own second factor.
...
See [`zeython.mfa`](reference/security.md) for the full function list —

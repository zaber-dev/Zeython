...
A plugin whose provider fails to import, or whose `register()`/`boot()`
raises, is not caught and skipped — it fails the app's boot the same way a
broken `app.register(...)` line in `main.py` would. A plugin silently
vanishing because of a typo in its own entry-point path, or a bug in its
`register()`, would be a much worse failure mode than a loud stack trace
at startup.
...
`discover_plugins()` is the plain function `PluginServiceProvider` calls
internally — useful on its own for a `zeython about`-style diagnostic, or
in a test asserting a particular plugin is (or isn't) installed. See
[CLI Reference](cli.md#project-introspection).
